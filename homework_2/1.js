    ...
    return new Promise(function (resolve, reject) {
        setTimeout(function () {
            ...
            if (outcome === 'win') {
                ...
                messageBuffer.push('Вы не нашли ягоды, но зато нашли Lit Energy (https://clck.ru/3E27gP).');
            }
            resolve();
        }, 2000);
    });
...
function findCaves() {
    return new Promise(function (resolve, reject) {
        setTimeout(function () {
            if (player.inventory.includes('Потерянный меч')) {
                messageBuffer.push('Вы уже нашли оружие Потерянный меч.')
            } else {
                const outcome = Math.random() > 0.75 ? 'win' : 'lose';
...
async function playGame() {
    while (true) {
        ...
        location.choices.forEach(function (choice, index) {
...
function getPlayerChoice(maxChoices) {
    return new Promise(function (resolve, reject) {
        rl.question('Сделайте выбор: ', function (input) {
...
function confirmExit() {
    return new Promise(resolve => {
        rl.question('Вы уверены, что хотите выйти? Весь прогресс будет утерян. (y/n): ', function (input) {
